...
def main():
    ...
    # Path options.
    parser.add_argument("--pretrained_model_path",type=str,default="pre/eniremodel.bin",
                        help="Path of the pretrained model.")
    parser.add_argument("--output_model_path", default="./models/chinese_bert_base.bin", type=str,
                        help="Path of the output model.")
    parser.add_argument("--vocab_path", type=str, default="models/google_uncased_en_vocab.txt",
                        help="Path of the vocabulary file.")
    parser.add_argument("--train_path", type=str,default="datasets/thucnews/train_new.tsv",
                        help="Path of the trainset.")
    parser.add_argument("--dev_path", type=str,default="datasets/thucnews/dev_new.tsv",
                        help="Path of the devset.") 
    parser.add_argument("--test_path", type=str,default="datasets/thucnews/test_new.tsv",
    ...
    parser.add_argument("--sub_layers_num", type=int, default=1, help="The number of subencoder layers.")#调试这个层数
    ...
    parser.add_argument("--distill_epochs_num", type=int, default=1,
    ...
    def evaluate(args, is_test, fast_mode=False):
        ...
        if not args.mean_reciprocal_rank:
            ...
            for i in range(confusion.size()[0]):
                print(confusion[i,i].item(),confusion[i,:].sum().item(),confusion[:,i].sum().item(),confusion[i,i].item())
                try:
                    A=confusion[i,i].item()
                    B=confusion[i,:].sum().item()
                    C=confusion[:,i].sum().item()
                    if A==0:
                        A=1
                    if B==0:
                        B=1
